...
# PrettyTable
TABLE = PrettyTable(["Region", "Lambda function(s)", "Code Storage", "Regional Concurrency", "Unreserved Concurrency"])
...
def function_metadata():
    ...
    for region_name in AWS_REGIONS:
        try:
            ...
            UNRESERVED_CONCURRENCY.append(response["AccountLimit"]["UnreservedConcurrentExecutions"])
...
def lambda_handler(event, context):
    ...
    for i, j in enumerate(ALL_REGIONS):
        TABLE.add_row([ALL_REGIONS[i], FUNCTION_COUNT[i], CODE_STORAGE[i], CONCURRENCY[i], UNRESERVED_CONCURRENCY[i]])
    print(TABLE)
